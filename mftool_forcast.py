...
def forecast_with_prophet(nav_df):
    # Prepare data for Prophet
    df_prophet = nav_df.rename(columns={'date': 'ds', 'nav': 'y'})
...
page = st.sidebar.selectbox("Go to", ("Home", "Mutual Fund Analysis","Scheme Codes"))
...
if page == "Home":
    ...
    mode = st.sidebar.selectbox("Mode", ("Analysis Mode", "Comparison Mode"))
    ...
    for scheme_code in fund_codes:
        ...
        if not nav_df.empty:
            ...
            with expander:
                ...
                st.subheader("Forecasting NAV with Prophet")
                forecast = forecast_with_prophet(nav_df)
                st.write(forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail())
                st.plotly_chart(go.Figure([go.Scatter(x=forecast['ds'], y=forecast['yhat'], name='Forecast'),
                                           go.Scatter(x=forecast['ds'], y=forecast['yhat_lower'], fill='tonexty', mode='none', name='Lower Bound'),
                                           go.Scatter(x=forecast['ds'], y=forecast['yhat_upper'], fill='tonexty', mode='none', name='Upper Bound')]),
